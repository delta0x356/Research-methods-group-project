...
* 1. M&A: Drop entire firm if it ever records an M&A (codes 200-299)
bysort permno: egen has_merger = max(inrange(dlstcd, 200, 299))
drop if has_merger == 1
drop has_merger

* 2. Delistings: Drop specific daily observations (flagged with delisting codes - liquidation and dropped (400-599))
bysort permno: egen has_dropped = max(inrange(dlstcd,400,599))
drop if has_dropped == 1
drop has_dropped

* 3. Ordinary Common Shares: Drop stokcs that are not common ordinary stocks (keep only stocks with share code of 10 or 11)
keep if inrange(shrcd, 10, 11)

* 3. THE $1 "MOVING STOCKS" FILTER (Firm-level reassignment)
bysort permno: egen price_ever_below_1 = min(abs(prc))
replace control = 1 if price_ever_below_1 < 1
replace g1 = 0 if price_ever_below_1 < 1
replace g2 = 0 if price_ever_below_1 < 1
replace g3 = 0 if price_ever_below_1 < 1
replace group = "C" if price_ever_below_1 < 1
drop price_ever_below_1

* 5. DAILY OBSERVATION FILTER
*  Drop a firm-day observation if the average daily price... is below $2"
* Note: 'prc' in CRSP is closing price. Ensure you use the correct variable 
* if your dataset has a specific 'average daily price' or use (bid+ask)/2.
drop if abs(prc) < 2
