...
* Input data lives in the v2.6/data_and_results subfolder
local datadir "v2.6/data_and_results"

* All output files (tables, figures) go to the results folder
local outdir "results"
cap mkdir "`outdir'"

* Verify required data files exist before starting
capture confirm file "`datadir'/Treatmentcontrollist.csv"
if _rc {
    di as error "Error: Treatmentcontrollist.csv not found in `c(pwd)'/`datadir'"
    exit 601
}

capture confirm file "`datadir'/dataset.dta"
if _rc {
    di as error "Error: dataset.dta not found in `c(pwd)'/`datadir'"
    exit 601
}
...
import delimited "`datadir'/Treatmentcontrollist.csv", delimiter(";") clear
rename *, lower
...
esttab control g1 g2 g3 ///
    ...
    title("Table 1: Summary Statistics by Treatment Group (Pre-Treatment Period, Mar–Aug 2016)") ///
    note("Statistics computed for the pre-treatment window. Groups defined by SEC pilot assignment. SmallSpread = 1 if firm-level average quoted spread < \$0.05 in the six months before treatment.") ///
    replace
restore
...
* Format: Step | Test | Comparison | Statistic | Value | Interpretation
...
file write ac `"8A,Parallel Trends — Pre-trend F-test,G1 vs Control,F-statistic (df=`df_g1'),F=`F_g1'  p=`p_g1',"p>0.10 supports parallel trends""' _n
file write ac `"8A,Parallel Trends — Pre-trend F-test,G2 vs Control,F-statistic (df=`df_g2'),F=`F_g2'  p=`p_g2',"p>0.10 supports parallel trends""' _n
file write ac `"8A,Parallel Trends — Pre-trend F-test,G3 vs Control,F-statistic (df=`df_g3'),F=`F_g3'  p=`p_g3',"p>0.10 supports parallel trends""' _n
file close ac
...
file write ac `"8C,Multicollinearity — Pairwise correlation,G1 x Treatment  vs  G1 x Treatment x SmallSpread,Pearson r,`rc_g1t',"Concern if |r|>0.90""' _n
file write ac `"8C,Multicollinearity — Pairwise correlation,G2 x Treatment  vs  G2 x Treatment x SmallSpread,Pearson r,`rc_g2t',"Concern if |r|>0.90""' _n
file write ac `"8C,Multicollinearity — Pairwise correlation,G3 x Treatment  vs  G3 x Treatment x SmallSpread,Pearson r,`rc_g3t',"Concern if |r|>0.90""' _n
file write ac `"8C,Multicollinearity — Pairwise correlation,G1 x Post-Pilot  vs  G1 x Post-Pilot x SmallSpread,Pearson r,`rc_g1p',"Concern if |r|>0.90""' _n
file write ac `"8C,Multicollinearity — Pairwise correlation,G2 x Post-Pilot  vs  G2 x Post-Pilot x SmallSpread,Pearson r,`rc_g2p',"Concern if |r|>0.90""' _n
file write ac `"8C,Multicollinearity — Pairwise correlation,G3 x Post-Pilot  vs  G3 x Post-Pilot x SmallSpread,Pearson r,`rc_g3p',"Concern if |r|>0.90""' _n
file close ac
...
esttab H1_Turnover H2_Turnover ///
    ...
    title("Impact of Tick Size on Log Turnover (Dependent variable: ln_turnover)") ///
    note("Firm and date fixed effects absorbed. Standard errors clustered at the firm level." ///
         "H1 (col 1): baseline DiD — Gi x Treatment coefficients are the average treatment effect for group i." ///
         "H2 (col 2): adds triple interactions — Gi x Treatment is the effect for firms with SmallSpread=0 (tick not binding);" ///
         "Gi x Treatment x SmallSpread is the ADDITIONAL effect for firms with SmallSpread=1 (pre-spread < \$0.05, tick IS binding)." ///
         "SmallSpread = 1 if firm-level average quoted spread in the six months before treatment was below the new tick size." ///
...
esttab H1_Volume H2_Volume ///
    ...
    title("Impact of Tick Size on Log Volume — Robustness (Dependent variable: ln_volume)") ///
    note("Firm and date fixed effects absorbed. Standard errors clustered at the firm level." ///
         "H1 (col 1): baseline DiD. H2 (col 2): adds triple interactions with SmallSpread." ///
         "SmallSpread = 1 if firm-level average quoted spread before treatment was below the new tick size." ///
         "* p<0.10  ** p<0.05  *** p<0.01") ///
    replace
