...
* 7. Penny Stocks: Drop DAILY OBSERVATIONS where the absolute price falls below $1
* We do not drop the whole firm to avoid selection bias, nor do we apply the $2 ex-ante SEC rule ex-post.
drop if abs(prc) < 1

* 8. Restrict sample to the defined study window (Jan 1, 2016 – Apr 30, 2019)
...
* 2. NUOVO: Define post-pilot period (L'esperimento è finito, si torna a $0.01)
